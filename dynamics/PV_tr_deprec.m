...
function  [qdd, nu, tau_apply, a, Xa, v, q_acc_X_fun, tau_X_fun] = PV_tr( model, q, qd, tau, f_ext, A_con, m_con, EEs )
...
if length(EEs) > 0
    ...
    for i = 1:length(EEs)
        ...
        E_cstr{EEs{i}} = SX(size(A_con{i},2), size(A_con{i},2)); %SX(size(A_con,2), size(A_con,2));
...
%         end
        ...
        if size( F_cstr{i}, 2) > 0
            %PV
            FS = F_cstr{i}'*S{i};
            if size(F_cstr{model.parent(i)}, 1) == 0
                ...
                zero_off_diag_LM = SX.zeros(size(E_cstr{model.parent(i)}, 1), size(E_cstr{i}, 2));
        ...
        if size(F_cstr{i}, 2) > 0
            ...
            if size(F_cstr0, 2) == 0
                ...
                zero_off_diag_LM = SX.zeros(size(E_cstr0,1), size(E_cstr{i},2));
...
if size(A_con, 2) > 0
    ...
    E_cstr0 = E_cstr0 + eye(size(E_cstr0))*1e-6;
