...
function  [qdd, nu, a_ee] = PV_tree_early( model, q, qd, tau, f_ext, K_con, k_con, Soft)
...
for i = model.NB:-1:1
    ...
    if model.parent(i) ~= 0
        ...
        if size( KA{i}, 1) > 0 && counter{i} > 0
            ...
            counter{model.parent(i)} = max(-1, counter{model.parent(i)} + counter{i});
        end
...
for i = 1:model.NB
    ...
    a{i} = a{i} + S{i}*qdd(i);
    if counter{i} == 0
        lambda{i} = back_sub(early_Lchol{i}', forward_sub(early_Lchol{i}, (lA{i} + KA{i}*a{i})));
    end
end
