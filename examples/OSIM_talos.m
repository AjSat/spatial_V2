...
[~, efp_osim_val, ~, ~] = efp_osim_fun(xfb_rand, qrand, Krand, krand);
[~, pv_osim_val, ~, ~] = pv_invosim_fun(xfb_rand, qrand, Krand, krand);

% Verifying the equality of OSIM using EFPA and PV-OSIM
assert(full(DM(sqrt(sumsqr(efp_osim_val - pv_osim_val)))) < 1e-10)
...
lambda_sol_from_pv_osim = inv(pv_osim_val)*krand;
